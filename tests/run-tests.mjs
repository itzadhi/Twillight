...
import { publicConfig, providerOptions } from "../src/web/server.mjs"
...
assert.equal(normalizePetName("dragom"), "sprite")
assert.equal(petAccess("dragon", false).allowed, true)
assert.equal(petAccess("dragon", true).activePet.title, "Twillight Companion")
assert.equal(petSidebarLine("dragon", { isDeveloper: true, processing: false }), "companion steady")
assert.equal(petSidebarLine("sprite", { isDeveloper: false, processing: true }), "companion working")
...
assert.equal(skillList().some((skill) => skill.id === "vercel-ai-sdk"), true)
assert.equal(skillList().some((skill) => skill.id === "ai-elements"), true)
...
const directScriptWorkflow = planLocalWorkflow(state, "write javascript script name tool.js")
assert.equal(directScriptWorkflow.steps.length, 1)
assert.equal(directScriptWorkflow.steps[0].tool, "write_file")
assert.equal(directScriptWorkflow.steps[0].input.path, "tool.js")
assert.equal(directScriptWorkflow.steps[0].input.content.includes("function main"), true)
const helloHtmlWorkflow = planLocalWorkflow(state, "create simple hello world html file")
assert.equal(helloHtmlWorkflow.steps.length, 1)
assert.equal(helloHtmlWorkflow.steps[0].tool, "write_file")
assert.equal(helloHtmlWorkflow.steps[0].input.path, "hello.html")
assert.equal(helloHtmlWorkflow.steps[0].input.content.includes("Hello World"), true)
...
assert.equal(closestCommand("/dragom"), "/pet")
assert.equal(closestCommand("/ai-sd"), "/ai-sdk")
assert.equal(closestCommand("/elemnts"), "/ai-elements")
assert.equal(closestCommand("/vercel-workflow"), "/vercel-workflows")
...
assert.equal(normalizeSlashInput("/dragon"), "/pet")
assert.equal(normalizeSlashInput("/sdk"), "/ai-sdk")
assert.equal(normalizeSlashInput("/elements"), "/ai-elements")
assert.equal(normalizeSlashInput("/workflow"), "/vercel-workflows")
...
const groqAuthError = await providerHttpError(new Response(JSON.stringify({ error: { message: "Invalid API Key" } }), { status: 401, statusText: "Unauthorized" }), "gsk_bad", { provider: "groq", endpoint: "https://api.groq.com/openai/v1/chat/completions" })
assert.equal(groqAuthError.authFailed, true)
...
const webPublicConfig = publicConfig({ provider: "cloudflare", model: "@cf/openai/gpt-oss-20b", permissionMode: "workspace", agentMode: "plan", enabledTools: "all", commandAllowlist: "node,npm", fallbackModels: "@cf/openai/gpt-oss-20b", cloudflareGatewayUrl: "https://ai.example.com", updateCheck: true, autoUpdate: false, webSound: true, soundTheme: "crystal", soundVolume: 42 })
assert.equal(webPublicConfig.provider, "cloudflare")
assert.equal(webPublicConfig.model, "@cf/openai/gpt-oss-20b")
assert.equal(webPublicConfig.commandAllowlist, "node,npm")
assert.equal(webPublicConfig.soundVolume, 42)
